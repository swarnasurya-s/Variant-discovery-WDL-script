...
command {
wc -l ${trimmed_R1} > R1_num_lines.txt ; wc -l ${trimmed_R2} > R2_num_lines.txt
}
}
...
workflow variant_discovery_project{
  ...
  call table { input: sorted_bam=sort_bam.sorted_bam, bamIndex=sort_bam.sorted_bam_index }
  call bqsr { input: recal_data=table.table_output, sorted_bam=sort_bam.sorted_bam, bamIndex=sort_bam.sorted_bam_index }
  call haplotypeCaller {input: bamIndex=sort_bam.sorted_bam_index, inputBAM=sort_bam.sorted_bam }
